 ...
 * @module
 */
export default function CircularRingLabels() {
	...
	function my(selection) {
		...
		if (typeof ownLabels !== "undefined") {
			for (let i = 0; i < radData.length; i++ ) {
				if (ownLabels[radData[i]] % 10 === 0 && ownLabels[radData[i]] != 0 && ownLabels[radData[i]] != 90) {
					displayLabels.push(ownLabels[radData[i]])
				} else {
					displayLabels.push(null)
				}
			}
		} else {
			displayLabels = [];
