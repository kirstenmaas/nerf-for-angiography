...
import HeatmapRing from "./HeatmapRing";
import CircularRingLabels from "./CircularRingLabels";
import CircularSectorLabels from "./CircularSectorLabels";
 ...
 */
export default function HeatmapRadial() {
	...
	function my(selection) {
		...
		selection.each(function(data) {
			...
			// Circular Labels
			const circularSectorLabels = CircularSectorLabels()
				.ownLabels(sectorLabels)
				.radius(radius * 1.04)
				.radialScale(xScale)
				.textAnchor("start");
			...
			// Ring Labels
			const circularRingLabels = CircularRingLabels()
				.ownLabels(ringLabels)
				.radialScale(yScale)
				.textAnchor("middle");
	...
	/**
	 * Sector labels Getter / Setter
	 *
	 * @param {} _v - Sector labels
	 * @returns {*}
	 */
	my.sectorLabels = function(_v) {
		if (!arguments.length) return sectorLabels;
		sectorLabels = _v;
		return this;
	};

	/**
	 * ringLabels labels Getter / Setter
	 *
	 * @param {} _v - Sector labels
	 * @returns {*}
	 */
	my.ringLabels = function(_v) {
		if (!arguments.length) return ringLabels;
		ringLabels = _v;
		return this;
	};
