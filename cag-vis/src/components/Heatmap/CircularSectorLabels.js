 ...
 * @module
 */
export default function CircularSectorLabels() {
	...
	function my(selection) {
		...
		const tickData = function() {
			...
			return tickArray.map((d, i) => {
				let label = d;
				console.log(ownLabels === 'undefined');
				if (typeof ownLabels !== 'undefined') {
					label = typeof ownLabels[d] !== 'undefined' ? ownLabels[d] : -ownLabels[d+3.14];
				} else {
					label = '';
				}
		...
		textSelect.enter()
			...
			.text((d) => {
				const text = d.label + '°';
				// skip if label is multiple of 5 (except 10s)
				return text.length > 2 && !text[text.length - 2].includes('5') ? text : null;
			})
			.attr("startOffset", (d) => d.label.toString().length == 1 ? d.offset + "%" : d.offset - d.extra + "%")
			.attr("id", (d) => d.value)
			.merge(textSelect);
	 ...
	 *
	 * @param {number} _v - Labels in array.
	 * @returns {*}
	 */
	my.ownLabels = function(_v) {
		if (!arguments.length) return ownLabels;
		ownLabels = _v;
		return this;
	};
